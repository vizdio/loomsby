...
async function createPost(boardId: string, title: string, content_md: string, image_url?: string): Promise<void> {
  const { error } = await supabase.from('posts').insert({ board_id: boardId, title, content_md, image_url: image_url ?? null })
  if (error) throw error
}
