...
async function sendMessage(threadId: string, content: string, image_url?: string): Promise<void> {
  const { error } = await supabase.from('chat_messages').insert({ thread_id: threadId, content, image_url: image_url ?? null })
  if (error) throw error
}
...
async function setTyping(threadId: string, isTyping: boolean): Promise<void> {
  const { error } = await supabase.from('chat_typing').upsert({ thread_id: threadId, is_typing: isTyping })
  if (error) throw error
}
