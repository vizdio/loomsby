...
export function PostEditor({ onSubmitPost }: { onSubmitPost: (input: PostInput) => Promise<void> }) {
  ...
  return (
    ...
    >
      ...
      {formState.errors.content_md && <p className="error">{formState.errors.content_md.message}</p>}
      <button type="submit" disabled={formState.isSubmitting}>Publish</button>
    </form>
  )
}
